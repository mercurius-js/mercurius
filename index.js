...
const plugin = fp(
  async function (app, opts) {
    ...
    if (gateway) {
      ...
      if (gateway.pollingInterval !== undefined) {
        if (typeof gateway.pollingInterval === 'number') {
          ...
          app.log.warn(
            `Expected a number for 'gateway.pollingInterval', received: ${typeof gateway.pollingInterval}`
          )
        }
      }
    ...
    fastifyGraphQl.extendSchema = function (s) {
      ...
      if (opts.federationMetadata) {
        throw new MER_ERR_INVALID_METHOD(
          'Calling extendSchema method when federationMetadata is enabled is not allowed'
        )
      }
    ...
    async function fastifyGraphQl (source, context, variables, operationName) {
      ...
      if (!cached) {
        ...
        const validationErrors = validate(fastifyGraphQl.schema, document, [
          ...specifiedRules,
          ...validationRules
        ])
