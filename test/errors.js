...
test('errors - should default to HTTP Status Code `200 Bad Request` if multiple errors are present and no data in the response', async (t) => {
  t.plan(2)
...
test('errors - should default to HTTP Status Code `200 Internal Server Error` if single error present but no status code defined', async (t) => {
  t.plan(2)
