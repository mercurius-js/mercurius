...
const { createPersistedQueryExecutor } = require('./persistedQuery')
...
module.exports = async function (app, opts) {
  ...
  const persistedQueryProvider = app.persistedQuery?.provider ?? {}
  ...
  app.post(
    ...
    async function (request, reply) {
      ...
      if (allowBatchedQueries && Array.isArray(request.body)) {
        ...
        return Promise.all(
          request.body.map(async (r, operationId) => {
            ...
            try {
              ...
              const { response } = errorFormatter(
                { errors: [toGraphQLError(e)] },
                request[kRequestContext]
              )
              return response
            }
  ...
  if (opts.ide) {
    ...
    if (
      opts.ide === true ||
      opts.ide === 'graphiql' ||
      (typeof opts.ide === 'object' &&
        opts.ide !== null &&
        opts.ide.enabled !== false)
    ) {
      ...
      app.get('/graphiql/config.js', (req, reply) => {
        ...
        idePlugins.forEach((plugin) => {
          if (plugin.name) {
            configRows.push(
              `window.GRAPIHQL_PLUGIN_${plugin.name.toUpperCase()} = ${JSON.stringify(
                plugin
              )}`
            )
        ...
        configRows.push(
          `window.GRAPHIQL_PLUGIN_LIST = ${JSON.stringify(plugins)}`
        )
