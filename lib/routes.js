...
module.exports = async function (app, opts) {
  ...
  const automaticPersistedQueries = {}

  async function execute (body, request, reply) {
    let { query } = body
    const { persisted, operationName, variables, extensions } = body

    if (onlyPersisted || persisted) {
      query = persistedQueries[query]
    }

    if (enableApolloAPQ) {
      if (query) {
        // If a query has been provided, store its hash for the next APQ request
        const sha256Hash = crypto.createHash('sha256').update(query, 'utf8').digest('hex')
        automaticPersistedQueries[sha256Hash] = query
      } else {
        // Otherwise, extract the query from automatically persisted queries
        const { persistedQuery } = (extensions || {})
        const { sha256Hash, version } = (persistedQuery || {})
        if (version !== 1 || !sha256Hash) {
          return new BadRequest('Persisted Query Version Not Supported')
        }

        query = automaticPersistedQueries[sha256Hash]

        if (!query) {
          // Return an error compatible with apollo-client, such that it will
          // re-send the full query
          return new Error('PersistedQueryNotFound')
        }
      }
    }
