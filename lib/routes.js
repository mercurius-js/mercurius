...
function tryJSONParse (request, value) {
  try {
    ...
    throw new FST_ERR_GQL_VALIDATION(err.message)
  }
}
...
module.exports = async function (app, opts) {
  ...
  async function executeQuery (query, variables, operationName, request, reply) {
    ...
    if (!query) {
      return new FST_ERR_GQL_PERSISTED_QUERY_NOT_FOUND('Unknown query')
    }
    ...
    if (persisted) {
      ...
      if (hash) {
        ...
        if (!query) {
          ...
          throw new FST_ERR_GQL_PERSISTED_QUERY_NOT_FOUND(notFoundError)
        }
        ...
        throw new FST_ERR_GQL_PERSISTED_QUERY_NOT_SUPPORTED(notSupportedError)
      }
    }
