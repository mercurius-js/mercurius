...
const { checkCSRFPrevention, normalizeCSRFConfig } = require('./csrf')
...
module.exports = async function (app, opts) {
  ...
  const csrfConfig = normalizeCSRFConfig(opts.csrfConfig)
  ...
  async function executePersistedQuery (body, request, reply) {
    ...
    const shouldPersistQuery = isPersistedQueryRetry && isPersistedQueryRetry(body) && query
    let hash
    let hashForQuery

    // Verify the hash before executing the retried query
    if (shouldPersistQuery) {
      hash = getHash && getHash(body)
      hashForQuery = getHashForQuery && getHashForQuery(query)
  ...
  const getOptions = {
    ...
    onRequest: async function (request, reply) {
      csrfConfig && checkCSRFPrevention(request, csrfConfig)
    },
