...
module.exports = async function (app, opts) {
  ...
  async function execute (body, request, reply) {
    ...
    if (enableAPQ) {
      if (query) {
        const hash = apqSettings.hashQuery(query)
        automaticPersistedQueries[hash] = query
      } else {
        const hash = apqSettings.getHash(extensions)

        if (!hash) {
          throw new Error(apqSettings.notSupportedError)
        }

        query = automaticPersistedQueries[hash]

        if (!query) {
          // Return an error compatible with apollo-client, such that it will
          // re-send the full query
          throw new Error(apqSettings.notFoundError)
        }
      }
    }
