          ...
          if (serviceForFieldType !== serviceForType) {
            if (serviceForType === null) {
              ...
              if (type.name === 'Subscription') {
                console.log('creating subscription fn')
                field.subscribe = makeSubscriptionResolver(serviceMap[serviceForFieldType])
              } else {
...
async function buildGateway (gatewayOpts, app, subscriber) {
  const { services } = gatewayOpts

  const serviceMap = await buildServiceMap(services, subscriber)
  ...
  app.decorateReply(kEntityResolvels)
  app.addHook('onRequest', async function (req, reply) {
    reply[kEntityResolvels] = factory.create()
  })
    ...
    factory.add(`${service}Entity`, async (queries, context) => {
      ...
      const mergedQueries = queries.reduce((acc, curr) => {
        if (!acc[curr.query]) {
          ...
          resultIndexes[q.indexOf(curr.query)] = [queryIndex]
        } else {
