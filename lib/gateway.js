...
function makeResolveQuery (service) {
  return function (root, args, context, info) {
    ...
    const selections = JSON.parse(JSON.stringify(fieldNodes))
      ...
      definitions: [{
        ...
        selectionSet: {
          ...
          selections: [{
            ...
            selectionSet: {
              kind: Kind.SELECTION_SET,
              selections: removeNonServiceTypeFields(selections[0].selectionSet.selections, service, type.name)
            }
          }]
        }
      }]
...
function createOperationForEntitiesQuery (inlineFragmentOnType, selections) {
  return {
    kind: Kind.DOCUMENT,
    definitions: [{
      kind: Kind.OPERATION_DEFINITION,
...
function getOperationForFieldResolving (fragmentType, fieldName, selections) {
  return createOperationForEntitiesQuery(fragmentType, [{
    kind: Kind.FIELD,
    name: {
      kind: Kind.NAME,
      value: fieldName
...
function makeFieldResolver (service, typeForFragment) {
  return function (parent, args, context, info) {
    ...
    const operation = getOperationForFieldResolving(typeForFragment, fieldName, selections)
...
function makeReferenceResolver (service) {
  return function (parent, args, context, info) {
    ...
    const operation = createOperationForEntitiesQuery(type, selections)
