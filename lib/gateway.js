...
function buildExecute (serviceMap) {
  return function execute (schema, documentAST, root, context, variables, operationName) {
    console.log(JSON.stringify(documentAST, null, 2))
...
async function buildGateway (gatewayOpts) {
  ...
  return {
    schema,
    serviceMap,
    execute: buildExecute(schema, serviceMap)
  }
}
