...
const { addErrorsToExecutionResult, MER_ERR_GQL_GATEWAY_REFRESH } = require('./errors')
const { preExecutionHandler } = require('./handlers')
  ...
  for (const [service, serviceDefinition] of Object.entries(serviceMap)) {
    ...
    factory.add(`${service}Entity`, async (queries) => {
      ...
      await Promise.all(Object.entries(mergedQueries).map(async ([query, variables], queryIndex, entries) => {
        // TODO: Trigger per service preExecution for entities here
        // TODO: optimise in the handler
        const preExecution = {}
        const request = { schema: serviceDefinition.schema, document: parse(query), context: queries[queryIndex].context }
        await preExecutionHandler(request, preExecution)
        const newQuery = print(request.document)
        ...
        response.json = addErrorsToExecutionResult(response.json, preExecution.errors)
