...
async function buildGateway (gatewayOpts, app) {
  ...
  for (const [service, serviceDefinition] of Object.entries(serviceMap)) {
    ...
    factory.add(`${service}Entity`, async (queries, context) => {
      ...
      const responses = await Promise.all(Object.entries(mergedQueries).map(([query, variables]) => {
        return serviceDefinition.sendRequest({
          method: 'POST',
          body: JSON.stringify({
            query,
            variables
      ...
      const result = responses.map(response => {
        return response.json.data._entities.map(entity => ({
          ...response,
          json: {
            data: {
              _entities: [entity]
