...
const errors = {
   ...
   * General errors
   */
  FST_ERR_INVALID_OPTS: createError(
    'FST_ERR_INVALID_OPTS',
    'Invalid options: %s'
  ),
  FST_ERR_METHOD_NOT_ALLOWED: createError(
   ...
   */
  FST_ERR_GQL_INVALID_SCHEMA: createError(
    'FST_ERR_GQL_INVALID_SCHEMA',
    'Invalid schema: check out the .errors property on the Error'
  ),
  FST_ERR_GQL_VALIDATION: createError(
    'FST_ERR_GQL_VALIDATION',
    'Graphql validation error',
    400
  ),
  FST_ERR_GQL_QUERY_DEPTH: createError(
    'FST_ERR_GQL_QUERY_DEPTH',
    '`%s query depth (%s) exceeds the query depth limit of %s`'
  ),
  /**
   * Gateway errors
   */
  FST_ERR_GQL_GATEWAY: createError(
    'FST_ERR_GQL_GATEWAY',
    'Gateway issues: %s'
  ),
  FST_ERR_GQL_GATEWAY_INVALID_SCHEMA: createError(
    'FST_ERR_GQL_GATEWAY_INVALID_SCHEMA',
    'The _entities resolver tried to load an entity for type "%s", but no object type of that name was found in the schema'
  ),
  FST_ERR_GQL_GATEWAY_REFRESH: createError(
    'FST_ERR_GQL_GATEWAY_REFRESH',
    'Refresh schema issues'
  ),
  /**
   * Persisted query errors
   */
  FST_ERR_GQL_PERSISTED_QUERY_NOT_FOUND: createError(
    'FST_ERR_GQL_PERSISTED_QUERY_NOT_FOUND',
    '%s',
    400
  ),
  FST_ERR_GQL_PERSISTED_QUERY_NOT_SUPPORTED: createError(
    'FST_ERR_GQL_PERSISTED_QUERY_NOT_SUPPORTED',
    '%s',
    400
  ),
  /**
   * Subscription errors
   */
  FST_ERR_GQL_SUBSCRIPTION_CONNECTION_NOT_READY: createError(
    'FST_ERR_GQL_SUBSCRIPTION_CONNECTION_NOT_READY',
    'Connection is not ready'
  ),
  FST_ERR_GQL_SUBSCRIPTION_FORBIDDEN: createError(
    'FST_ERR_GQL_SUBSCRIPTION_FORBIDDEN',
    'Forbidden'
  ),
  FST_ERR_GQL_SUBSCRIPTION_UNKNOWN_EXTENSION: createError(
    'FST_ERR_GQL_SUBSCRIPTION_UNKNOWN_EXTENSION',
    'Unknown extension %s'
  ),
  FST_ERR_GQL_SUBSCRIPTION_MESSAGE_INVALID: createError(
    'FST_ERR_GQL_SUBSCRIPTION_MESSAGE_INVALID',
