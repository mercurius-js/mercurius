...
function toGraphQLError (err, extensions) {
  const gqlError = new GraphQLError(
    ...
    err.positions,
    err.path,
    err instanceof GraphQLError ? err.originalError : err,
  ...
  if (err.errors) {
    errors = err.errors.map((error) => {
      ...
      if (error.originalError?.errors) {
        error.message = error.originalError.errors[0].message
        error.locations = error.originalError.errors[0].locations
        return toGraphQLError(error, error.originalError.errors[0].extensions)
      }
