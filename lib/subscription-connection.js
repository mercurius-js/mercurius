...
const { FST_ERR_GQL_SUBSCRIPTION_FORBIDDEN, FST_ERR_GQL_SUBSCRIPTION_UNKNOWN_EXTENSION } = require('./errors')
...
module.exports = class SubscriptionConnection {
    ...
    if (extensions && extensions instanceof Array) {
      for (const extension of extensions) {
        switch (extension.type) {
          ...
          default:
            throw new FST_ERR_GQL_SUBSCRIPTION_UNKNOWN_EXTENSION(extension.type)
        }
      }
    }
  ...
  async handleConnectionInitExtension (extension) {
    if (typeof this.onConnect === 'function') {
      ...
      if (!authorize) {
        throw new FST_ERR_GQL_SUBSCRIPTION_FORBIDDEN()
      }
      return authorize
    }
