    ...
    if (extensions && extensions instanceof Array) {
      for (const extension of extensions) {
        switch (extension.type) {
          ...
          default:
            throw new MER_ERR_GQL_SUBSCRIPTION_UNKNOWN_EXTENSION(
              extension.type
            )
        }
      }
    ...
    if (operationAST.operation === 'subscription') {
      ...
      for await (const value of subIter) {
        ...
        if (this.context.onSubscriptionResolution !== null) {
          try {
            await onSubscriptionResolutionHandler({
              execution: value,
              context
            })
          } catch (error) {
      ...
      // execute Query or Mutation (errors are still caught and handled by the caller of this function)
