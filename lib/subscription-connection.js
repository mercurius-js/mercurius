...
module.exports = class SubscriptionConnection {
  constructor (socket, {
    ...
    this.protocol = getProtocolByName(socket.protocol)
  ...
  async handleMessage (message, isBinary) {
    ...
    try {
      ...
      this.sendMessage(this.protocol.GQL_ERROR, null, 'Message must be a JSON string')
      return
    }

    const { id, type } = data
    switch (type) {
      case this.protocol.GQL_CONNECTION_INIT:
        await this.handleConnectionInit(data)
        break
      case this.protocol.GQL_CONNECTION_TERMINATE:
        this.handleConnectionClose()
        break
      case this.protocol.GQL_START: {
        if (this.isReady) {
          this.handleGQLStart(data).catch(e => {
            this.sendMessage(this.protocol.GQL_ERROR, id, e.message)
          })
        } else {
          this.sendMessage(this.protocol.GQL_CONNECTION_ERROR, undefined, { message: 'Connection has not been established yet.' })
          return this.handleConnectionClose()
        }
        break
      }
      case this.protocol.GQL_STOP:
        await this.handleGQLStop(data)
        break
      default:
        this.sendMessage(this.protocol.GQL_ERROR, id, 'Invalid payload type')
    }
  }
  ...
  async handleConnectionInit (data) {
    ...
    if (typeof this.onConnect === 'function') {
      ...
      if (!authorize) {
        this.sendMessage(this.protocol.GQL_CONNECTION_ERROR, undefined, { message: 'Forbidden' })
        return this.handleConnectionClose()
      }
    ...
    this.sendMessage(this.protocol.GQL_CONNECTION_ACK)
    this.isReady = true
  }
  ...
  async handleGQLStart (data) {
    ...
    for await (const value of subIter) {
      ...
      this.sendMessage(this.protocol.GQL_DATA, data.id, value)
    }

    this.sendMessage(this.protocol.GQL_COMPLETE, data.id, null)
  }
