...
const { FST_ERR_GQL_SUBSCRIPTION_MESSAGE_INVALID, FST_ERR_GQL_SUBSCRIPTION_CONNECTION_NOT_READY } = require('./errors')
...
class SubscriptionClient {
  ...
  async handleMessage (message) {
    ...
    try {
      ...
      throw new FST_ERR_GQL_SUBSCRIPTION_MESSAGE_INVALID(`"${message}" must be JSON parsable.`)
    }
    ...
    switch (data.type) {
      ...
      default:
        /* istanbul ignore next */
        throw new FST_ERR_GQL_SUBSCRIPTION_MESSAGE_INVALID(`Invalid message type "${data.type}"`)
    }
  }
  ...
  startOperation (operationId) {
    ...
    if (!started) {
      if (!this.ready) {
        throw new FST_ERR_GQL_SUBSCRIPTION_CONNECTION_NOT_READY()
      }
