 ...
 *
 */
function makeResolver ({ service, createOperation, transformData, isQuery, isReference, isSubscription, typeToServiceMap, serviceMap }) {
  return async function (parent, args, context, info) {
    ...
    if (isQuery) {
      ...
      if (typeToServiceMap) {
        ...
        const targetService = typeToServiceMap[transformed.__typename]
        ...
        if (targetService && targetService !== service.name) {
          ...
          operation = createEntityReferenceResolverOperation({
            returnType: transformed.__typename,
            selections,
            variableDefinitions: []
          })
          ...
          variables.representations = [{
            ...removeNonIdProperties(transformed, schema.getType(transformed.__typename)),
            ...getRequiredFields(transformed, schema.getType(transformed.__typename))
          }]
          ...
          Object.assign(transformed, response2.json.data._entities[0])
        }
      }
      return transformed
    }
