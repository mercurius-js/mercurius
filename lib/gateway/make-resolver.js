        ...
        if (selection.kind === Kind.INLINE_FRAGMENT) {
          ...
          return {
            ...
            selectionSet: {
              ...
              selections: removeNonServiceTypeFields([...selection.selectionSet.selections, ...requiredFields], service, inlineFragmentType, schema)
            }
          }
        }
...
function makeResolver ({ service, createOperation, transformData, isQuery, isReference, isSubscription }) {
  return async function (parent, args, context, info) {
    ...
    if (cached) {
      ...
      query = cached.query
      // TODO: work out whether to set in the cache or parse the query here
    ...
    if (isQuery) {
      // TODO: Trigger per service preExecution
      // TODO: trigger new query if present
      const { modifiedQuery } = await preExecutionHandler({ schema, document: operation, context })
