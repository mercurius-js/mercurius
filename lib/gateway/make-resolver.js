    ...
    if (isQuery) {
      ...
      if (typeToServiceMap) {
        ...
        if (targetService && targetService !== service.name) {
          ...
          await Promise.all(toFill.map(async transformed => {
            ...
            variables.representations = [{
              ...removeNonIdProperties(transformed, schema.getType(transformedTypeName)),
              ...getRequiredFields(transformed, schema.getType(transformedTypeName))
            }]
