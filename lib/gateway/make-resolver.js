  ...
  return async function (parent, args, context, info) {
    ...
    if (isSubscription) {
      ...
      if (context.preGatewaySubscriptionExecution !== null) {
        await preGatewaySubscriptionExecutionHandler({ schema, document: operation, context, service: { name: service.name } })
      }
    ...
    if (isQuery) {
      ...
      if (context.preGatewayExecution !== null) {
        ({ modifiedQuery } = await preGatewayExecutionHandler({ schema, document: operation, context, service: { name: service.name } }))
      }
