...
const kEntityResolvels = Symbol('fastify-gql.entity-resolvers')
...
function makeResolver ({ service, createOperation, transformData, isQuery, isReference }) {
  return function (parent, args, context, info) {
    ...
    const { reply, __currentQuery, lruGatewayResolvers } = context
    ...
    if (isQuery) {
      ...
      return reply[kEntityResolvels][`${service.name}Entity`]({
        query,
        variables,
        originalRequestHeaders: reply.request.headers
      }).then(transformData)
...
function makeSubscriptionResolver (service) {
  return async function (root, args, { pubsub }, info) {
    const subscriptionId = service.createSubscription('subscription { commentAdded(postId: "p1") { cid text } }', {})
    const subscription = await pubsub.subscribe(`${service.name}_${subscriptionId}`)
