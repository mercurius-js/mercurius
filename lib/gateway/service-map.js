...
const createClient = require('./socket-connection')
...
async function buildServiceMap (services, subscriber) {
  ...
  for (const service of services) {
    ...
    if (service.wsUrl) {
      const client = createClient(service.wsUrl, subscriber, service.name)

      const subscriptionQueryMap = {}
      let idCount = 0

      serviceConfig.createSubscription = (query, args) => {
        const subscriptionString = JSON.stringify({ query, args })
        let id = subscriptionQueryMap[subscriptionString]
        ...
        subscriptionQueryMap[subscriptionString] = id
