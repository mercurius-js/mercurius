...
async function hookRunner (functions, runner, request, cb) {
  ...
  async function next (err) {
    ...
    let result
    try {
      result = runner(functions[i++], request)
    } catch (error) {
      next(error)
      return
    }
...
function hookIterator (fn, request) {
  return fn(request.schema, request.document, request.context)
}

function preParsingHookIterator (fn, request) {
  return fn(request.schema, request.source, request.context)
}

function onResolutionHookIterator (fn, request) {
