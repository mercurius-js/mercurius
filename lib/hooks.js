...
async function preExecutionHooksRunner (functions, request, executionResult) {
  for (const fn of functions) {
    const result = await fn(request.schema, request.document, request.context)

    if (result) {
      if (typeof result.document !== 'undefined') {
        request.document = result.document
        request.modifiedQuery = true
      }
      if (typeof result.errors !== 'undefined') {
        if (executionResult.errors !== undefined) {
          executionResult.errors = executionResult.errors.concat(result.errors)
        } else {
          executionResult.errors = result.errors
        }
      }
    }
  }
