...
function Hooks () {
  ...
  this.onSubscriptionConnectionClose = []
  this.onSubscriptionConnectionError = []
  this.onExtendSchema = []
}
...
function assignLifeCycleHooksToContext (context, hooks) {
  ...
  if (hooks.onSubscriptionConnectionClose.length > 0) contextHooks.onSubscriptionConnectionClose = hooks.onSubscriptionConnectionClose.slice()
  if (hooks.onSubscriptionConnectionError.length > 0) contextHooks.onSubscriptionConnectionError = hooks.onSubscriptionConnectionError.slice()
...
function preSubscriptionParsingHookRunner (fn, request) {
  return fn(request.schema, request.source, request.context, request.id)
}

function preSubscriptionExecutionHookRunner (fn, request) {
  return fn(request.schema, request.document, request.context, request.id)
}

function onSubscriptionResolutionHookRunner (fn, request) {
  return fn(request.execution, request.context, request.id)
}

function onSubscriptionEndHookRunner (fn, request) {
  return fn(request.context, request.id)
}

function onConnectionCloseHookRunner (fn, request) {
  return fn(request.context, request.code, request.reason)
}

function onConnectionErrorHookRunner (fn, request) {
  return fn(request.context, request.error)
}
